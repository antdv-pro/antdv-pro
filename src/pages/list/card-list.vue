...
<template>
  <page-container>
    ...
    <a-row :gutter="16">
      ...
      <a-col :xs="16" :sm="8" :md="6" :lg="6" :xl="6" class="mb-4" v-for="(item, index) in data" :key="index">
        <a-card :bordered="false"
          ...
          <template #default>
            <div class="flex h-27">
              ...
              <div class="ml">
                <div style="font-size: 18px; font-weight: 500;">{{ item.title }}</div>
                <div class="h-17 overflow-hidden overflow">{{  content  }}</div>
              </div>
            </div>
          </template>
